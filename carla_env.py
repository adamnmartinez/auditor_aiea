...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    self.discrete_act = params['discrete_steer'] # acc, steer
    # self.n_acc = len(self.discrete_act[0])
    self.n_steer = len(self.discrete_act)
    if self.discrete:
      ...
      self.action_space = spaces.Box(np.array([params['continuous_steer_range'][0]]), np.array([params['continuous_steer_range'][1]]), dtype=np.float32)  # just steer
  ...
  def reset(self):
    ...
    def get_radar_data(data, point_list):
      ...
      intensity_col = 1.0 - np.log(intensity + epsilon) / np.log(np.exp(-0.004 * 100))
      int_color = np.c_[
  ...
  def step(self, action):
    ...
    if self.discrete:
      steer = self.discrete_act[action]
    else:
      #acc = action[0]
      #steer = action[1]
      steer = action
    ...
    act = carla.VehicleControl(steer=float(-steer))
  ...
  def _try_spawn_ego_vehicle_at(self, transform):
    ...
    if not overlap:
      ...
      vehicle.enable_constant_velocity(carla.Vector3D(1, 0, 0))
